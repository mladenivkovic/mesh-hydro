...
void io_read_cmdlineargs(int argc, char* argv[]){
  ...
  if (argc < 3){
    throw_error("Too few arguments given. Run this program with ./hydro paramfile datafile\n");
  }
  else {
...
void io_read_toutfile(){
  ...
  while (fgets(tempbuff, MAX_LINE_SIZE, par)) {
    ...
    if (nlines > 1){
      if (value <= past_value) throw_error("While reading output time file '%s':"
...
void check_number_of_columns_IC(char* line, int should){
  ...
  if (splits != should){
    throw_error("I expected %d columns, I got %d; Line was %s", should, splits, line);
  }
}
