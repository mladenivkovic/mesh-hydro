...
void solver_prepare_flux_computation(cell* left, cell* right, int dim) {
  ...
  if (riemann_has_vacuum(&left->prim, &right->prim, dim)) {
    riemann_get_full_vacuum_solution_for_WAF(
      &left->prim,
      &right->prim,
      left->Sk,
      left->riemann_fluxes,
...
#if RIEMANN == HLLC
    riemann_get_hllc_full_solution_for_WAF(
      &left->prim,
      &right->prim,
      left->Sk,
      left->riemann_fluxes,
...
#else
    riemann_get_full_solution_for_WAF(
      &left->prim,
      &right->prim,
      left->Sk,
      left->riemann_fluxes,
...
void solver_compute_cell_pair_flux(
  cell* left, cell* right, float* dt, int dim
) {
  /* --------------------------------------------------------------------
  ...
  float phi[3] = {
    1., 1., 1.
  }; /* set the default value for the no limiter situation */
  ...
  for (int k = 0; k < 3; k++) {
    ...
    fluxsum.E += s * psi
                 * (left->riemann_fluxes[k + 1].E - left->riemann_fluxes[k].E);
  }
  ...
  left->cflux.rho
    = 0.5
      * (left->riemann_fluxes[0].rho + left->riemann_fluxes[3].rho - fluxsum.rho);
  left->cflux.rhou[0] =
