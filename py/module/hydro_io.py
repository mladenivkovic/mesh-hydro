...
def read_output(fname):
    ...
    if ndim == 1:
        rho, u, p = np.loadtxt(fname, usecols=[1, 2, 3], dtype=np.float, unpack=True, skiprows=linecount)

    elif ndim == 2:
        rho, ux, uy, p = np.loadtxt(fname, usecols=[2, 3, 4, 5], dtype=np.float, unpack=True, skiprows=linecount)
...
def read_twostate_ic(fname, nx):
    ...
    for val, name in [(rhoL, "rhoL"), (uL, "uL"), (pL, "pL"), (rhoR, "rhoR"), (uR, "uR"), (pR, "pR") ]:
    ...
    while True:
        ...
        if line_is_empty(clean):
            ...
            if ftstr.strip() != "filetype":
                raise ValueError("First non-empty non-comment line is '{0}', but is should contain the filetype.".format(clean))
            ftclean = ftype.strip()
